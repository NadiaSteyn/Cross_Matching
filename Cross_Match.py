...
# Function to work out the xy cartesian distance. (only works for cartesian - in decimal degrees). 
...
# Trystan's function. Gives angular separation of two sources
## In the case that we want to use the ra/dec/vel we need to calculate the angular separation. This is the explicit formulism
...
# Function which takes a point (x,y,z) and finds the closest point in x_array,y_array,z_array GIVEN some xy lim and then z lim. Weighted towards the xy lim (checks xy lim first). 
...
def Cross_match(x1,y1,z1,x2,y2,z2,xy_lim,z_lim,frame='cartesian'):
    ...
    idx_1 = []  #this is the index for the x1,y1,z1 arrays (c)
    idx_2 = [] #this is the corresponding index for x2,y2,z2 (catalog)
    d2d = [] #this is the on sky distances (either in cartesian units or degrees depending on frame choice)
    d3d = [] #this is the line-of-sight distance (z or Vel) which will be in whatever units the user inputs
